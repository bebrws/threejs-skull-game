...
async function main() {
    ...
    // Idea here is from : https://stackoverflow.com/questions/67656871/render-a-shader-without-having-to-add-a-geometry-to-the-scene-in-threejs
    // Trying to figure out how to use a shader on the whole scene
    // This stackoverflow is how to use a shader on a background "scene" basically
    // I want the scene rendered, maybe to a plane, then apply the shader on it?
    var bgGeom = new THREE.PlaneBufferGeometry(2, 2);
    var bgMesh = new THREE.Mesh(bgGeom, new THREE.ShaderMaterial({
        uniforms,
        vertexShader,
        fragmentShader,
    }));
